...
class GZIP:
	...
	def decompress(self):
		...
		while not BFINAL == 1:
			...
			# Function that returns the values of HLIT, HDIST, and HCLEN according to the structure of each block
			def block_size():
			...
			# Function that stores the code lengths based on the previously obtained HCLEN
			def array_block(HCLEN):
				block_order = [16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15]
				result = [0] * len(block_order)
				for i in range(HCLEN):
					result[block_order[i]] = self.readBits(3)
				return result
			
			# Function that creates a histogram of the array and returns an array with 2 columns (column 0 == symbols and column 1 == occurrences). 
			# This function takes into account the occurrence of all values from 0 up to the highest symbol.
			def histogram(array):
				values, occurrences = np.unique(array, return_counts=True)
				symbols_histogram = np.zeros((np.max(values) + 1, 2), dtype=int)
				symbols_histogram[:, 0] = np.arange(np.max(values) + 1)
				symbols_histogram[values, 1] = occurrences
				return symbols_histogram
			
			# Based on the previously created histogram, this function converts the previously obtained codes into Huffman codes 
			# from the "code length alphabet"
			def convert(result_array):
				occurrences = histogram(result_array)
				code = 0
				occurrences[0,1] = 0
				next_code = np.zeros(len(occurrences), dtype= int)
				for j in range (1, len(occurrences)):
					code = (code + occurrences[j - 1, 1]) << 1
					next_code[j] = code
					
				code_list = np.zeros(len(result_array), dtype= int)
			
				for j in range(len(result_array)):
					comprimento = result_array[j]
					if (comprimento != 0):
						code_list[j] = next_code[comprimento]
						next_code[comprimento] += 1
				return code_list
			
			# Using the array from the array_block function and the code generated by the convert function, 
			# this function generates a Huffman tree that stores all obtained data, 
			# taking into account the codes that require reading some extra bits.
			def build_tree(result_array, code):
				huffman_tree = HuffmanTree()
				for i in range (len(code)):
					if(result_array[i] != 0):
						string = bin(code[i])[2:]
						if(len(string) < result_array[i]):
							increments = '0' * (result_array[i] - len(string))
							string = increments + string
						huffman_tree.addNode(string, i, False)
				return huffman_tree
			
			# Based on the previously obtained tree and the lengths/distances and codes obtained in the previous functions, 
			# this function stores the literals/lengths, encoded in the last functions, into an array.
			def comp(HLIT, huffman_tree):
				array = []
				while (len(array) < HLIT):
					...
					while position == -2:
						bit = self.readBits(1)
						position = huffman_tree.nextNode(str(bit))
						if(position >= 0):
							break
						if(position == -1):
							print("erro")
			...
			# Function executed alongside the decompress function that adds to the data array the extra bits related to the backward distance 
			# and length, both of which were calculated in the decompress function.
			def decompress_data(data, comp, dist):
				x = len(data) - dist
				if(x >= 0):
					for i in range(comp):
						data.append(data[x])
			...
			# Function to decompress block data (data bytes) using LZ77 dictionaries, handling all extra bits correctly. 
			# It loops until the end of the block, reading the HLIT tree first, and a second tree if `pos` > 256, 
			# calculating length and distance as needed.
			def decompress(decompressed_data, HLITtree, HDISTtree):
				block_end = True
				...
				while(block_end):
					...
					if (pos == -1): 
						raise ValueError("Error in HLITtree")
					elif(pos < 256): 
						decompressed_data.append(pos)
					elif(pos == 256): 
						block_end = False
					elif(257 <= pos <= 285):
						if(257 <= pos <= 264 or pos == 285):
							...
							raise ValueError("Error reading the lenght")
							block_end = False
						...
						decompress_data(decompressed_data, comp, dist)
				return decompressed_data
			
			# Function used at the end of the decompression process to process the data before writing it to the file.
			def writeFile(data):
			...
			HLIT, HDIST, HCLEN = block_size()

			result_array = array_block(HCLEN)

			code = convert(result_array)
			
			huffman_tree = build_tree(result_array, code)
			array_HLIT = comp(HLIT, huffman_tree)

			array_HDIST = comp(HDIST, huffman_tree)

			array_HLITconvert = convert(array_HLIT)
			array_HDISTconvert = convert(array_HDIST)

			HLITtree = build_tree(array_HLIT, array_HLITconvert)
			HDISTtree = build_tree(array_HDIST, array_HDISTconvert)
	
			if(numBlocks == 0): decompressed_data = []
			decompressed_data = decompress(decompressed_data, HLITtree, HDISTtree)

			if(BFINAL == 1): writeFile(decompressed_data)
